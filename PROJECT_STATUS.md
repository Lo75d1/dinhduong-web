...
- **Gộp "Hồ sơ + Khuyến nghị & vi chất" thành 1 mục màu ở đầu khu Kết quả (2026-07-25):** trước đây "Khuyến nghị & vi chất" là 1 trong 3 nút popup; hồ sơ chỉ ở bước nhập. Nay thêm mục `<section>` nền xanh nhạt `#eaf3ee` **"Hồ sơ & đối chiếu nhu cầu"** đặt NGAY TRƯỚC `ClinicalSummary` ("Đọc nhanh khẩu phần"), gồm: tóm tắt hồ sơ chỉ-đọc (`ProfileSummary` — chips Giới/Tuổi/Cao/Nặng/BMI/Vận động/Sinh lý + nút "✏️ Sửa hồ sơ" → `setActiveView("entry")`), `RecommendationComparison`, `MicronutrientComparison`, `DietCodeComparison`. Đã bỏ popup "clinical" khỏi `RESULT_GROUPS` (còn 2 nút: Quy đổi, 10 biểu đồ → lưới `sm:grid-cols-2`). Lưu ý: các bảng con giờ render LUÔN trong khu Kết quả (trước chỉ khi mở popup) — đã test không crash khi API/DB lỗi.
- **Nút "Thực đơn đã lưu" (2026-07-25):** `ServerRationActions.tsx` trước chỉ có nút "Lưu lên server" — lưu được nhưng KHÔNG có UI xem/mở lại (API `GET /api/rations` + `GET /api/rations/[id]` đã có sẵn từ trước, chỉ thiếu giao diện). Đã thêm nút **📂 Thực đơn đã lưu** (hiện khi đã đăng nhập) mở modal liệt kê phiếu đã lưu (tên, số món, ngày cập nhật), mỗi phiếu có nút **Mở**. Mở lại = dựng `Row[]` từ `RationItem` (map `edibleGrams→grams`, `nutrientsJson→nutrients`, `classifyJson→classify`, `{...EMPTY_CLASSIFY}` bù thiếu), ghi vào localStorage (`khauphan_rows_v1`, `khauphan_profile_v1`, mode) rồi `window.location.reload()` để `MealInput`/`PersonalProfile` tự hydrate (state phân tán, không lift lên Calculator). Chưa có nút xóa (chưa có DELETE API) — để làm sau nếu cần.
- **Sửa hiển thị đăng nhập (2026-07-25, commit `96cfe24`):** `AccountMenu.tsx` (menu tài khoản ở header layout) trước là client component fetch `/api/auth/me` trong `useEffect` → vì nằm trong layout gốc (mount một lần) nên sau khi đăng nhập điều hướng về, effect không chạy lại, phải **tải lại trang** tên tài khoản mới hiện. Đã đổi sang **server component** đọc phiên trực tiếp bằng `getSessionUser()` (`lib/auth.ts`): render đúng ngay lần đầu, cập nhật theo `router.refresh()` mà `LoginForm` gọi sau đăng nhập. Có `.catch(() => null)` để CSDL lỗi tạm thời chỉ hiện nút "Đăng nhập" chứ không sập layout. Đã deploy VPS, xác nhận chạy đúng.
