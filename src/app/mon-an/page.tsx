...
export default async function MonAnPage({
  ...
  const [items, total, categories, ageGroups, diseaseGroups, foodMeals, foodMealTotal] = await Promise.all([
    prisma.dish.findMany({ where, orderBy: { name: "asc" }, skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE, select: { id: true, name: true, source: true, totalWeightG: true, servingUnit: true, categoryRaw: true, diseaseDiet: true, ageGroup: true, imageSourceId: true, _count: { select: { ingredients: true } } } }),
    ...
    prisma.food.findMany({ where: foodMealWhere, orderBy: { name: "asc" }, take: 30, select: { id: true, name: true, source: true, foodGroup: true, energyKcal: true, proteinG: true, lipidG: true, glucidG: true, imageUrl: true } }),
  ...
  if (age) exportParams.set("age", age);
  if (disease) exportParams.set("disease", disease);
  const exportHref = `/api/dishes/export${exportParams.toString() ? `?${exportParams.toString()}` : ""}`;
