...
export default function MultiDayAnalysis({
  ...
  return (
    <section className="mt-3 flex flex-col gap-4 rounded-xl border-2 border-[#7BAF9E] bg-[#F5FAF8] p-3 sm:p-4" aria-label="Phân tích thực đơn nhiều ngày">
      <header className="rounded-xl bg-[#123C36] p-4 text-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold tracking-[0.16em] text-[#BCE3D6]">BÁO CÁO DINH DƯỠNG TOÀN KỲ</p>
            <h2 className="mt-1 text-2xl font-black sm:text-3xl">{periodName}</h2>
            <p className="mt-1 text-sm text-[#D8EEE7]">Phân tích {analysis.dataDayCount}/{analysis.calendarDayCount} ngày có thực phẩm · Ngày trống không bị tính là 0 kcal.</p>
          </div>
          <div className="flex flex-wrap gap-2" data-no-print>
            <button type="button" onClick={() => download(buildMultiDayWordHtml(days, analysis, shopping, profile), "application/msword;charset=utf-8", "bao-cao-dinh-duong-toan-ky.doc")} className="rounded-md border border-white/50 bg-white px-3 py-2 text-sm font-bold text-[#123C36]">Xuất Word toàn kỳ</button>
            <button type="button" onClick={() => download(buildMultiDayExcelXml(days, analysis, shopping, profile), "application/vnd.ms-excel;charset=utf-8", "bao-cao-dinh-duong-toan-ky.xls")} className="rounded-md bg-[#F0B429] px-3 py-2 text-sm font-bold text-[#2F2500]">Xuất Excel toàn kỳ</button>
          </div>
        </div>
      </header>
      ...
      <AnalysisSection eyebrow="01 · NĂNG LƯỢNG" title="Tổng kỳ và biến động từng ngày" note="Ngưỡng đạt: 90–110% nhu cầu ngày. Chênh lệch dương là vượt, âm là thiếu.">
        ...
        <div className="mt-3 overflow-x-auto rounded-lg border border-[#B8CFC7]">
      ...
      <AnalysisSection eyebrow="03 · BỮA ĂN" title="Phân bổ năng lượng và P/L/G theo bữa" note="Tổng bữa được cộng trên toàn bộ ngày có dữ liệu; một bữa có thể chứa vài chục món mà không giới hạn dòng.">
        <details className="mb-3 rounded-lg border border-[#B8CFC7] bg-[#F7FBF9]" open={!sharesValid}>
          <summary className="cursor-pointer px-3 py-2 text-sm font-bold text-[#123C36]">⚙ Mục tiêu năng lượng từng bữa {sharesValid ? `(đủ ${round(shareTotal)}%)` : `(hiện ${round(shareTotal)}%)`}</summary>
          <div className="border-t border-[#D6E5DF] p-3">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {mealNames.map((meal) => <label key={meal} className="flex items-center gap-1 rounded border border-[#B8CFC7] bg-white px-2 py-1 text-xs"><span className="max-w-40 truncate">{meal}</span><input aria-label={`Tỷ lệ mục tiêu ${meal}`} inputMode="decimal" value={shareText[meal] ?? ""} onChange={(event) => setShareText((current) => ({ ...current, [meal]: event.target.value }))} className="w-14 rounded border border-neutral-300 px-1 py-0.5 text-right" /><span>%</span></label>)}
              <button type="button" onClick={splitEvenly} className="rounded border border-[#176B57] px-2 py-1 text-xs font-bold text-[#123C36]">Chia đều</button>
      ...
      <details className="rounded-xl border border-[#B8CFC7] bg-white">
        <summary className="cursor-pointer px-4 py-3 text-base font-black text-[#123C36]">🛒 Bảng đi chợ gộp toàn kỳ ({shopping.length} thực phẩm)</summary>
        <div className="overflow-x-auto border-t border-[#D6E5DF]">
          <table className="w-full min-w-[760px] text-sm"><thead className="bg-[#E2F0EB] text-[#123C36]"><tr><th className="px-3 py-2 text-left">Thực phẩm</th><th className="px-3 py-2 text-right">Sống sạch</th><th className="px-3 py-2 text-right">Mua/xuất kho</th><th className="px-3 py-2 text-right">Thải bỏ</th><th className="px-3 py-2 text-left">Dùng trong ngày</th></tr></thead><tbody>{shopping.map((item) => <tr key={item.key} className="border-t border-[#DDE8E4]"><td className="px-3 py-2 font-medium">{item.foodName}</td><td className="px-3 py-2 text-right">{formatNumber(item.edibleGrams, "g")}</td><td className="px-3 py-2 text-right">{formatNumber(item.rawGrams, "g")}</td><td className="px-3 py-2 text-right">{formatNumber(item.wastePercent, "%")}</td><td className="px-3 py-2 text-xs">{item.days.join(", ")}</td></tr>)}</tbody></table>
        </div>
      </details>
    </section>
  );
...
function AnalysisSection({ eyebrow, title, note, children }: { eyebrow: string; title: string; note: string; children: React.ReactNode }) {
  return <section className="rounded-xl border border-[#B8CFC7] bg-white p-3 sm:p-4"><div className="mb-3 border-b-2 border-[#176B57] pb-2"><p className="text-[11px] font-black tracking-[0.13em] text-[#176B57]">{eyebrow}</p><h3 className="mt-0.5 text-lg font-black text-[#123C36] sm:text-xl">{title}</h3><p className="mt-1 text-xs text-neutral-600 sm:text-sm">{note}</p></div>{children}</section>;
}

function BigMetric({ title, value, note, status }: { title: string; value: string; note: string; status?: EnergyStatus }) {
  return <div className="rounded-xl border border-[#B8CFC7] bg-white p-4 shadow-sm"><div className="flex items-center justify-between gap-2"><p className="text-xs font-bold uppercase tracking-wide text-neutral-600">{title}</p>{status && <StatusBadge status={status} />}</div><p className="mt-2 text-2xl font-black text-[#123C36]">{value}</p><p className="mt-1 text-xs text-neutral-600">{note}</p></div>;
}

function SmallMetric({ title, value }: { title: string; value: string }) {
  return <div className="rounded-lg border border-[#D6E5DF] bg-[#F7FBF9] p-3"><p className="text-xs text-neutral-600">{title}</p><p className="mt-1 text-xl font-black text-[#123C36]">{value}</p></div>;
}
...
function MacroCard({ macro }: { macro: MacroPeriodSummary }) {
  ...
  return <article className="rounded-xl border border-[#B8CFC7] p-4"><div className="flex items-center justify-between"><h4 className="text-lg font-black text-[#123C36]">{macro.label}</h4><StatusBadge status={macro.status} /></div><div className="mt-3 grid grid-cols-2 gap-2 text-sm"><div><p className="text-xs text-neutral-500">Tổng kỳ</p><p className="font-bold">{formatMetric(macro.grams, "g")}</p></div><div><p className="text-xs text-neutral-500">Trung bình/ngày</p><p className="font-bold">{formatNumber(macro.averageGramsPerDay, "g")}</p></div><div><p className="text-xs text-neutral-500">Năng lượng</p><p className="font-bold">{formatNumber(macro.energyKcal, "kcal")}</p></div><div><p className="text-xs text-neutral-500">Tỷ lệ năng lượng</p><p className="font-bold">{formatNumber(macro.energySharePct, "%")}</p></div></div><div className="mt-3 h-3 overflow-hidden rounded-full bg-neutral-100"><div className="h-full rounded-full" style={{ width: `${width}%`, background: color }} /></div><p className="mt-2 text-xs text-neutral-600">Khuyến nghị: {recommendedRange}{macro.gramsPerKgPerDay != null ? ` · ${round(macro.gramsPerKgPerDay)} g/kg/ngày` : ""}</p></article>;
}

function NutrientRow({ nutrient }: { nutrient: NutrientPeriodSummary }) {
  return <tr className="border-t border-[#DDE8E4]"><td className="px-3 py-2 font-bold text-[#123C36]">{nutrient.label}</td><td className="px-3 py-2 text-right">{formatMetric(nutrient.total, nutrient.unit)}</td><td className="px-3 py-2 text-right">{formatNumber(nutrient.averagePerDay, nutrient.unit)}</td><td className="px-3 py-2 text-right">{formatNumber(nutrient.targetPerDay, nutrient.unit)}</td><td className="px-3 py-2 text-right">{formatNumber(nutrient.targetPeriod, nutrient.unit)}</td><td className="px-3 py-2 text-right">{formatNumber(nutrient.percentOfTarget, "%")}</td><td className="px-3 py-2 text-right">{nutrient.comparableDays ? `${nutrient.achievedDays}/${nutrient.comparableDays}` : "—"}</td><td className="px-3 py-2 text-center"><StatusBadge status={nutrient.status} /></td><td className="px-3 py-2 text-xs text-neutral-600">{nutrient.total.incomplete ? "Thiếu dữ liệu ở một số thực phẩm" : nutrient.targetPerDay == null ? "Chưa có mốc RNI phù hợp" : nutrient.isUpperLimit ? "Đối chiếu giới hạn trên" : nutrient.targetType || "Đủ dữ liệu"}</td></tr>;
}

function DayDetail({ day }: { day: DayPeriodAnalysis }) {
  return <details className="rounded-lg border border-[#B8CFC7] bg-[#FBFDFC]" open={day.hasData}><summary className="cursor-pointer px-3 py-2"><span className="font-black text-[#123C36]">{day.label}</span><span className="ml-2 text-sm text-neutral-600">{formatNumber(day.energy.value, "kcal")} · {day.dishCount} món · {day.foodGroupCount ?? "—"} nhóm TP</span></summary><div className="overflow-x-auto border-t border-[#D6E5DF]"><table className="w-full min-w-[820px] text-sm"><thead className="bg-[#EAF4F0]"><tr><th className="px-3 py-2 text-left">Bữa</th><th className="px-3 py-2 text-right">Kcal</th><th className="px-3 py-2 text-right">Mục tiêu</th><th className="px-3 py-2 text-right">P (g)</th><th className="px-3 py-2 text-right">L (g)</th><th className="px-3 py-2 text-right">G (g)</th><th className="px-3 py-2 text-center">Đánh giá</th></tr></thead><tbody>{day.meals.map((meal) => <tr key={meal.meal} className="border-t border-[#DDE8E4]"><td className="px-3 py-2 font-bold">{meal.meal}</td><td className="px-3 py-2 text-right">{formatMetric(meal.energy)}</td><td className="px-3 py-2 text-right">{formatNumber(meal.targetKcal)}</td><td className="px-3 py-2 text-right">{formatMetric(meal.nutrients.proteinG)}</td><td className="px-3 py-2 text-right">{formatMetric(meal.nutrients.lipidG)}</td><td className="px-3 py-2 text-right">{formatMetric(meal.nutrients.glucidG)}</td><td className="px-3 py-2 text-center"><StatusBadge status={meal.status} /></td></tr>)}</tbody></table></div></details>;
}
